...
\operator{13}{Iterative Method Chaining} Similar to in MASC \opnumber{5} (Method Chains) this operator implements method chaining to hide the value. Where this operator differs from \opnumber{5} is that it can take a value specified by the user and create that many method calls. The method calls are then chained in sucession. Every method call will has a safe value until the final call which transforms the value into an unsafe value. This was created to test the limits of how far crypto detectors check method calls and can allow a user to determine where their fault point is. Just like \opnumber{5} this behavior would simulate a (T3) developer.
